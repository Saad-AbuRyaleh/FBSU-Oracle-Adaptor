...
public interface ZatcaHeaderErpRepository extends JpaRepository<ZatcaHeaderERP, Long> {
    Optional<List<ZatcaHeaderERP>> findAllByStatusAndInvoiceType(ZatcaStatus status, String invoiceType);

    Optional<ZatcaHeaderERP> findFirstByInvoiceIdOrderBySeqIdDesc(String invoiceId);

    @Transactional
    @Modifying
    @Query("update ZatcaHeaderErpRepository s  set s.status = :status, s.errorDetails=:errorMessage, s.reference=:invoiceqReference,s.payableRoundingAmount=:payableRoundingAmount where s.invoiceId = :invoiceId and s.seqId = :seqId")
    ...
    @Transactional
    @Modifying
    @Query("update ZatcaHeaderErpRepository s  set s.status = :status where s.customerTrxId = :customerTrxId and s.seqId = :seqId")
