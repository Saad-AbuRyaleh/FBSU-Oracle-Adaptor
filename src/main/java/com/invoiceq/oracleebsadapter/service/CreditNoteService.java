...
public class CreditNoteService {
    ...
    public void sendAndHandle(CreditNoteRequest request, String originalInvoiceId, Long customerTrxId, Long seqId) {
        ...
        try {
            ...
            zatcaHeaderErpRepository.updateZatcaStatus(seqId, originalInvoiceId, ZatcaStatus.TECHNICAL_FAILED, toJson(response.getErrors()), null, null);
            zatcaHeaderErpRepository.updateInvoiceStatusProcedure(customerTrxId, ZatcaStatus.TECHNICAL_FAILED.name(), toJson(response));
        }
    }
    ...
    private void handleInvoiceStatus(CreditNoteOperationResponse response, String invoiceId, Long customerTrxId, Long seqId) {
        try {
            if (response.getValid() && Objects.nonNull(response.getBody())) {
                ...
                zatcaHeaderErpRepository.updateInvoiceStatusProcedure(customerTrxId, ZatcaStatus.BUSINESS_FAILED.name(), toJson(response));
            }
        } catch (Exception e) {
    ...
    public void proceedInvoices() {
        Optional<List<ZatcaHeaderERP>> invoicesOptional = zatcaHeaderErpRepository.findAllByStatusAndInvoiceType(ZatcaStatus.HOST_READY, "381");
