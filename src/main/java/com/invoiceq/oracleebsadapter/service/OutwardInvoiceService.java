...
public class OutwardInvoiceService {
    ...
    public void proceedInvoices(){
        ...
        Optional<List<SaptcoZatcaHeaderERP>> invoicesOptional = saptcoZatcaHeaderErpRepository.findAllByStatusAndInvoiceType(ZatcaStatus.HOST_READY,"388");
    ...
    public void sendAndHandle(UploadOutwardInvoiceRequest request, String originalInvoiceId , Long customerTrxId,Long seqId){
        ...
        try {
            ...
            saptcoZatcaHeaderErpRepository.updateZatcaStatus(seqId,originalInvoiceId,ZatcaStatus.TECHNICAL_FAILED,toJson(iqEx.getResponseEntity()),null,null);
            saptcoZatcaHeaderErpRepository.updateInvoiceStatusProcedure(customerTrxId,ZatcaStatus.TECHNICAL_FAILED.name(),toJson(iqEx.getResponseEntity()));
        }
    }
    ...
    private void handleInvoiceStatus(OutwardInvoiceOperationResponse response, String invoiceId, Long customerTrxId,Long seqId){
        try {
            if(response.getValid() && Objects.nonNull(response.getBody())){
                ...
                saptcoZatcaHeaderErpRepository.updateInvoiceStatusProcedure(customerTrxId,ZatcaStatus.BUSINESS_FAILED.name(),toJson(response.getErrors()));
            }
        }catch (Exception e){
